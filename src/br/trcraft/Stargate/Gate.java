...
public class Gate {
	...
	public Gate(final String filename, final Character[][] layout, final HashMap<Character, Integer> types,
			final HashMap<Character, Integer> metadata) {
	...
	public void save(final String gateFolder) {
		try (final BufferedWriter bw = new BufferedWriter(new FileWriter(gateFolder + this.filename))) {
			...
			Stargate.log.log(Level.SEVERE, "Could not save Gate {0} - {1}",
					new Object[] { this.filename, ex.getMessage() });
		}
	}
			...
			for (int x = 0; x < this.layout[y].length; ++x) {
				...
				if (id == -2 || id == -4) {
					if (!Stargate.ignoreEntrance) {
						...
						if (!onCreate || type != Material.AIR.getId()) {
							if (type != this.portalBlockClosed && type != this.portalBlockOpen) {
								if (this.portalBlockOpen == Material.WATER.getId()
										|| this.portalBlockOpen == Material.STATIONARY_WATER.getId()) {
								...
								if (this.portalBlockOpen == Material.LAVA.getId()
										|| this.portalBlockOpen == Material.STATIONARY_LAVA.getId()) {
					...
					if (mData != null && topleft.modRelative(x, y, 0, modX, 1, modZ).getData() != mData) {
						Stargate.debug("Gate::Matches", "Block Data Mismatch: "
								+ topleft.modRelative(x, y, 0, modX, 1, modZ).getData() + " != " + mData);
						return false;
					}
				}
			}
	...
	public static Gate loadGate(final File file) {
				...
				if (designing) {
					...
					for (final Character symbol : line.toCharArray()) {
						if (symbol.equals('?') || !types.containsKey(symbol)) {
							Stargate.log.log(Level.SEVERE,
									"Could not load Gate {0} - Unknown symbol ''{1}'' in diagram",
									new Object[] { file.getName(), symbol });
							return null;
						}
						row.add(symbol);
					}
		...
		gate.toOwner = (config.containsKey("toowner") ? Boolean.valueOf(config.get("toowner"))
				: iConomyHandler.toOwner);
		if (gate.getControls().length != 2) {
			Stargate.log.log(Level.SEVERE, "Could not load Gate {0} - Gates must have exactly 2 control points.",
					file.getName());
			return null;
		}
		Gate.frameBlocks.addAll((Collection<? extends Integer>) frameTypes);
		gate.save(file.getParent() + "/");
		return gate;
	}

	private static int readConfig(final HashMap<String, String> config, final Gate gate, final File file,
			final String key, final int def) {
		if (config.containsKey(key)) {
			try {
				return Integer.parseInt(config.get(key));
			} catch (NumberFormatException ex) {
				Stargate.log.log(Level.WARNING,
	...
	public static void populateDefaults(final String gateFolder) {
		...
		final Character[][] layout = { { ' ', 'X', 'X', ' ' }, { 'X', '.', '.', 'X' }, { '-', '.', '.', '-' },
				{ 'X', '*', '.', 'X' }, { ' ', 'X', 'X', ' ' } };
		final HashMap<Character, Integer> types = new HashMap<Character, Integer>();
		types.put('.', -2);
		types.put('*', -4);
