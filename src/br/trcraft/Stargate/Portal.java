...
import br.trcraft.Stargate.event.StargateActivateEvent;
import br.trcraft.Stargate.event.StargateCloseEvent;
import br.trcraft.Stargate.event.StargateCreateEvent;
import br.trcraft.Stargate.event.StargateDeactivateEvent;
import br.trcraft.Stargate.event.StargateOpenEvent;
import br.trcraft.Stargate.event.StargatePortalEvent;
...
public final class Portal {
	...
	private Portal(final Blox topLeft, final int modX, final int modZ, final float rotX, final Blox id,
			final Blox button, final String dest, final String name, final boolean verified, final String network,
			final Gate gate, final String owner, final boolean hidden, final boolean alwaysOn, final boolean priv,
			final boolean free, final boolean backwards, final boolean show, final boolean noNetwork,
			final boolean random, final boolean bungee) {
		this.lastDest = "";
		this.owner = "";
		this.hidden = false;
		this.alwaysOn = false;
	...
	public boolean open(final Player openFor, boolean force) {
		...
		if (!this.isAlwaysOn()) {
			...
			if (!this.random && end != null
					&& (!end.isFixed() || end.getDestinationName().equalsIgnoreCase(this.getName())) && !end.isOpen()) {
	...
	public boolean isOpenFor(final Player player) {
		return this.isOpen && (this.isAlwaysOn() || this.player == null
				|| (player != null && player.getName().equalsIgnoreCase(this.player.getName())));
	}
	...
	public void teleport(final Vehicle vehicle) {
		final Location traveller = new Location(this.world, vehicle.getLocation().getX(), vehicle.getLocation().getY(),
		...
		if (passenger != null) {
			final Vehicle v = (Vehicle) exit.getWorld().spawn(exit, (Class) vehicle.getClass());
			vehicle.eject();
			vehicle.remove();
			...
			final Vehicle mc = (Vehicle) exit.getWorld().spawn(exit, (Class) vehicle.getClass());
	...
	public Location getExit(final Location traveller) {
		...
		if (this.gate.getExit() != null) {
			...
			loc = exit.modRelativeLoc(0.0, 0.0, 1.0, traveller.getYaw(), traveller.getPitch(), this.modX * back, 1,
					this.modZ * back);
		} else {
			Stargate.log.log(Level.WARNING, "[Stargate] Missing destination point in .gate file {0}",
					this.gate.getFilename());
		}
		if (loc != null) {
			if (this.getWorld().getBlockTypeIdAt(loc.getBlockX(), loc.getBlockY(), loc.getBlockZ()) == Material.STEP
					.getId()) {
	...
	public boolean isVerified() {
		...
		for (final RelativeBlockVector control : this.gate.getControls()) {
			this.verified = (this.verified
					&& this.getBlockAt(control).getBlock().getTypeId() == this.gate.getControlBlock());
		}
	...
	public boolean activate(final Player player) {
		...
		final StargateActivateEvent event = new StargateActivateEvent(this, player, this.destinations,
				this.destination);
		Stargate.server.getPluginManager().callEvent((Event) event);
		if (event.isCancelled()) {
			Stargate.activeList.remove(this);
	...
	public void cycleDestination(final Player player, final int dir) {
		...
		if (!this.isActive() || this.getActivePlayer() != player) {
			...
			Stargate.debug("cycleDestination",
					"Network Size: " + Portal.allPortalsNet.get(this.network.toLowerCase()).size());
	...
	public final void drawSign() {
		...
		if (sMat != Material.SIGN && sMat != Material.WALL_SIGN && sMat != Material.SIGN_POST) {
			...
			Stargate.debug("Portal::drawSign",
					"Block: " + this.id.getBlock().getType() + " @ " + this.id.getBlock().getLocation());
			return;
		}
		...
		if (!this.isActive()) {
			...
			if (index == max && max > 1 && ++done <= 3) {
				if (iConomyHandler.useiConomy() && iConomyHandler.freeGatesGreen) {
					...
					Stargate.setLine(sign, done,
							(green ? ChatColor.DARK_GREEN : "") + this.destinations.get(index - 2));
				} else {
			...
			if (index > 0 && ++done <= 3) {
				if (iConomyHandler.useiConomy() && iConomyHandler.freeGatesGreen) {
					...
					final boolean green = Stargate.isFree(this.activePlayer, this, dest2);
					Stargate.setLine(sign, done,
							(green ? ChatColor.DARK_GREEN : "") + this.destinations.get(index - 1));
				} else {
	...
	public void unregister(final boolean removeAll) {
		...
		if (this.id.getBlock().getType() == Material.WALL_SIGN && this.id.getBlock().getState() instanceof Sign) {
			final Sign sign = (Sign) this.id.getBlock().getState();
	...
	private Blox getBlockAt(final RelativeBlockVector vector) {
		return this.topLeft.modRelative(vector.getRight(), vector.getDepth(), vector.getDistance(), this.modX, 1,
				this.modZ);
	}
	...
	private void register() {
		...
		if (this.isBungee()) {
			...
			if (!Portal.lookupNamesNet.containsKey(this.getNetwork().toLowerCase())) {
				...
				Portal.lookupNamesNet.put(this.getNetwork().toLowerCase(), new HashMap<String, Portal>());
			}
			...
			if (!Portal.allPortalsNet.containsKey(this.getNetwork().toLowerCase())) {
				...
				Portal.allPortalsNet.put(this.getNetwork().toLowerCase(), new ArrayList<String>());
			}
	...
	public static Portal createPortal(final SignChangeEvent event, final Player player) {
		...
		for (final Gate possibility : possibleGates) {
			if (gate == null && buttonVector == null) {
				...
				for (final RelativeBlockVector vector : vectors) {
					final Blox tl = parent.modRelative(-vector.getRight(), -vector.getDepth(), -vector.getDistance(),
							modX, 1, modZ);
					if (gate == null) {
		...
		Stargate.debug("createPortal", "h = " + hidden + " a = " + alwaysOn + " p = " + priv + " f = " + free + " b = "
				+ backwards + " s = " + show + " n = " + noNetwork + " r = " + random + " u = " + bungee);
		...
		Blox button = null;
		Portal portal = null;
		portal = new Portal(topleft, modX, modZ, rotX, id, button, destName, name, false, network, gate,
				player.getName(), hidden, alwaysOn, priv, free, backwards, show, noNetwork, random, bungee);
		int cost = Stargate.getCreateCost(player, gate);
		final StargateCreateEvent cEvent = new StargateCreateEvent(player, portal, event.getLines(), deny, denyMsg,
				cost);
		...
		if (cost > 0) {
			if (!Stargate.chargePlayer(player, null, cost)) {
				...
				inFundMsg = Stargate.replaceVars(inFundMsg, new String[] { "%cost%", "%portal%" },
	...
	public static void saveAllGates(final World world) {
		...
		try (final BufferedWriter bw = new BufferedWriter(new FileWriter(loc, false))) {
			for (final Portal portal : Portal.allPortals) {
				...
				bw.append((CharSequence) builder.toString());
				bw.newLine();
			}
		} catch (Exception e) {
	...
	public static void loadAllGates(final World world) {
		...
		if (db.exists()) {
			...
			try {
						...
						if (!line.startsWith("#")) {
							...
							if (split.length < 8) {
								...
								if (!(sign.getBlock().getState() instanceof Sign)) {
									Stargate.log.log(Level.INFO,
											"[Stargate] Sign on line {0} doesn''t exist. BlockType = {1}",
											new Object[] { l, sign.getBlock().getType() });
								} else {
									...
									final Gate gate = split[7].contains(";") ? Gate.getGateByName("nethergate.gate")
											: Gate.getGateByName(split[7]);
									if (gate == null) {
										Stargate.log.log(Level.INFO,
												"[Stargate] Gate layout on line {0} does not exist [{1}]",
												new Object[] { l, split[7] });
									} else {
										...
										final boolean alwaysOn = split.length > 12
												&& split[12].equalsIgnoreCase("true");
										...
										final boolean backwards = split.length > 16
												&& split[16].equalsIgnoreCase("true");
										final boolean show = split.length > 17 && split[17].equalsIgnoreCase("true");
										final boolean noNetwork = split.length > 18
												&& split[18].equalsIgnoreCase("true");
										...
										final Portal portal = new Portal(topLeft, modX, modZ, rotX, sign, button, dest,
												name, false, network, gate, owner, hidden, alwaysOn, priv, free,
												backwards, show, noNetwork, random, bungee);
										portal.register();
										portal.close(true);
									}
								}
					...
					if (!portal2.wasVerified()) {
						if (!portal2.isVerified() || !portal2.checkIntegrity()) {
							for (final RelativeBlockVector control : portal2.getGate().getControls()) {
								if (portal2.getBlockAt(control).getBlock().getTypeId() != portal2.getGate()
										.getControlBlock()) {
									Stargate.debug("loadAllGates", "Control Block Type == "
											+ portal2.getBlockAt(control).getBlock().getTypeId());
								}
							}
				...
				Stargate.log.log(Level.INFO, "[Stargate] '{'{0}'}' Loaded {1} stargates with {2} set as always-on",
						new Object[] { world.getName(), portalCount, OpenCount });
			} catch (Exception e) {
				Stargate.log.log(Level.SEVERE, "Exception while reading stargates from {0}: {1}",
						new Object[] { db.getName(), l });
			}
		} else {
	...
	public String toString() {
		return String.format("Portal [id=%s, network=%s name=%s, type=%s]", this.id, this.network, this.name,
				this.gate.getFilename());
	}
	...
	public boolean equals(final Object obj) {
		...
		} else if (!this.network.equalsIgnoreCase(other.network)) {
			return false;
		}
		return true;
	}
	...
	static {
		lookupBlocks = new HashMap<Blox, Portal>();
		lookupEntrances = new HashMap<Blox, Portal>();
		lookupControls = new HashMap<Blox, Portal>();
		allPortals = new ArrayList<Portal>();
		allPortalsNet = new HashMap<String, ArrayList<String>>();
		lookupNamesNet = new HashMap<String, HashMap<String, Portal>>();
		bungeePortals = new HashMap<String, Portal>();
	}
}
